...
検索戦略によって合計500件のレポートが取得され、レビューされました。繰り返し、胎児心拍数モニターに関係のないレポート、および中央胎児モニターを含む4つのケースを排除した後、コアデータセットを形成する117のレポートが残されました。ソースは、これらのレポートのうち35件で「外国」としてリストされています。データセット内の117のFDAレポートはすべて、「米国FDA 2009-2019に報告された心電図関連の胎児および新生児の死亡」というプロジェクトタイトルの下で、オープンアクセスのFigshareデジタルリポジトリで入手できます[19]。合計117の内47ケース（40％）が母体心拍アーチファクトの可能性があると特定しました（これらの47ケースは、追加ファイル1で省略形で、全体がFigshareリポジトリのフォルダーにあります）[20]。製造業者の報告書は、未検出の母体心拍アーチファクトのリスクを軽減するために追加の措置を講じるべきであると頻繁に示唆しています：心電図検査前の代替手段による胎児の生命の確認（16例）、信号曖昧性検出技術の有効化（16例）、および/または、これらの推奨事項の少なくとも1つを使用して、合計117（25％）のうち合計29のケースで、信号曖昧性テクノロジー（10のケース）からのアラームへの応答を改善します（これらの29のケースは、追加ファイル2および完全にFigshareリポジトリのフォルダにあります）[21]。さまざまな胎児モニターからの報告がありました（表1）。
